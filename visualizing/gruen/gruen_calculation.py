...
class Gruen:
    def __init__(self, bert_cola_dir):
        ...
        saved_pretrained_cola_model_dir = 'storage/checkpoints/metrics/cola/' + model_name + '/'
        config_class, model_class, tokenizer_class = (BertConfig, BertForSequenceClassification, BertTokenizer)
        config = config_class.from_pretrained(saved_pretrained_cola_model_dir, num_labels=2, finetuning_task='CoLA')
        self.cola_tokenizer = tokenizer_class.from_pretrained(saved_pretrained_cola_model_dir, do_lower_case=0)
        self.cola_model = model_class.from_pretrained(saved_pretrained_cola_model_dir, from_tf=bool('.ckpt' in 'bert-base-cased'),
    ...
    def get_cola_score(self, sentences):
        ...
        cola_score = evaluate_cola(self.cola_model, candidates, self.cola_tokenizer, 'bert-base-cased')
    ...
    def get_focus_score(all_summary):
        def compute_sentence_similarity():
            ...
            @Language.factory("wmd_similarity")
            def my_component(nlp, name):
                return WMD.SpacySimilarityHook(nlp)

            nlp.add_pipe('wmd_similarity', last=True)
    ...
    def compute(self, predictions):
        processed_candidates = self.preprocess_candidates(predictions)
...
if __name__ == '__main__':
    candidates = ["This is a good example.",
                  "This is a bad example. It is ungrammatical and redundant. Orellana shown red card for throwing "
                  "grass at Sergio Busquets. Orellana shown red card for throwing grass at Sergio Busquets."]
    gruen = Gruen('storage/checkpoints/metrics/cola/')
    gruen_score = gruen.compute(candidates)
    print(gruen_score)
